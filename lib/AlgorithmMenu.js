...
const MENU_ITEMS = ['KCET', 'KCST', 'TDC', 'RAW'];
...
export default class AlgorithmMenu {
  ...
  constructor(clustersView) {
    ...
    const div = d3Selection.select('#algorithm-menu').classed('menu', true);
    this.svgBounds = div.node().getBoundingClientRect();
  ...
  addHeader() {
    ...
    g.append('text')
      .attr('x', (10 + ((this.svgWidth * 0.7) / 2)) - 55)
      .attr('y', 30)
