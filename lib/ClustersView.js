...
function drawGraph(object, data, clusterId) {
  object.hide(object, data, clusterId);
}

function callback(object, data, clusterId) {
  object.clinicalPath.draw(data, clusterId);
...
export default class ClustersView {
  ...
  printCircles() {
    ...
    node.append('circle')
      .on('click', d => drawGraph(that, getClusterData(that, d.data.clusterId), d.data.clusterId))
      .transition()
      .duration(800)
      .delay(800)
      .attr('r', d => d.r)
  ...
  hide(object, data, clusterId) {
    ...
    this.div.transition().delay(800)
      ...
      .on('end', () => {
        callback(object, data, clusterId);
      });
  }
