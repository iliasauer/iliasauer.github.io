...
export default class InteractiveBlock {
  ...
  _addCheckboxes(labels) {
    ...
    checkboxBlock.each(function () {
      const checkbox = d3Selection.select(this).append('input')
                .attr('type', 'checkbox')
                .attr('id', (d, i) => `checkbox_${i}`)
                .attr('class', CHECKBOX_ID)
                .property('value', d => d);
      checkboxes.push(checkbox);
      d3Selection.select(this).append('label')
  ...
  _addRadioButtons() {
    ...
    const radioButtons = [];
    for (const field of radioButtonFields) {
  ...
  static __updateRadioButtons(clickedButton) {
    d3Selection.select(`#${RADIO_BUTTONS_BLOCK_ID}`).selectAll(`.${RADIO_BUTTON_ID}`)
