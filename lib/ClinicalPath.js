...
export default class ClinicalPath {
  ...
  collectLinks(patients) {
    for (const patient in patients) {
      this.buildPaths(patients[patient].path);
    }
  ...
  updatePaths(comboValue, sliderValue) {
    ...
    if (comboValue !== null) {
      ...
      this.svg.selectAll('.link').classed('patient-selected', d => Util.isPathSelected(comboValue, d));
    }
  }
