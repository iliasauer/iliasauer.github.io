export default class Util {
  ...
  static collectSinks(result, sink, transformedPaths) {
    if (sink in result) {
      ...
      for (const transformedPath of transformedPaths) {
        let existed = false;
        for (const sinkPath of sinkPaths) {
          if (transformedPath.source === sinkPath.source && transformedPath.target === sinkPath.target) {
            existed = true;
            break;
          }
        }
  ...
  static isPathSelected(paths, data, fakeSourceId) {
    for (const i in paths) {
      const patientPath = paths[i];
      if ((patientPath.source === data.source.id && patientPath.target === data.target.id) || (data.source.id === fakeSourceId)) {
        return true;
      }
    }
    return false;
